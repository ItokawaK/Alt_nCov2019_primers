...
parser = argparse.ArgumentParser(description='This tool trims portion of primer from aligned reads. '
                                              ...
                                              'Without --r1 and --r2 options, the program output interleaved fastq data '
                                              'to STDOUT, which may be piped directly to bwa mem (e.g. \'bwa mem -p db -)\'')
parser.add_argument('primer_bed', help='bed file describing primer coorinates')
parser.add_argument('--r1', dest='fastq_1', help='name of output fastq file 1')
parser.add_argument('--r2', dest='fastq_2', help='name of output fastq file 2')
...
for sam_line in sys.stdin:
    ...
    if alignment_bucket[0] and alignment_bucket[1]:
        ...
        if args.fastq_1 and args.fastq_2:
